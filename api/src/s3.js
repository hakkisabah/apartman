...
const uploadfile = async (req,res)=>{
  ...
  if (!req.files || Object.keys(req.files).length === 0 || sampleFile.mimetype !== process.env.ACCEPT_MIMETYPE) {
    return res.status(500).json({error:'No files were uploaded or wrong file extension'})
  }
...
const addItem = async (req,res)=>{
  ...
  const items = itm.addItem(itm.createInvoice({donem,sene,giderTuru,tutar}))
  ...
  await upload(params,(err)=>{
    if (err) return res.status(500).json({error:'Upload failed',reason:err});
...
const updateItem = async (req,res)=>{
  ...
  if (sampleFile !== ''){ // every time check and if file exists upload it
    await uploadfile(req,res)
  }
  ...
  try {
    ...
    s3.deleteObject(getObjectparams, async (err,data)=>{
      if (!err){
        ...
        params.Key = `${process.env.UPLOAD_FOLDER}${process.env.ITEMS_FILE_NAME}.json`
        await upload(params,(err)=>{
          if (err) return res.status(500).json({error:'After delete update failed',reason:err});
